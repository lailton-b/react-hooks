...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=lailton_b%40hotmail.com).

"This lesson had a lot of things to learn, but it mainly focused on explaining about how React Hooks treat states and rerenders the function component. The awesome concept of lazy initialization was presented, showing how you are able to improve performance by calling an expensive expression only when the component is mounted. That's achieavable when you create a function and calls it as an useState initialValue, what happens is that useState treats callback functions as a way to only instance it once. But of course that wasn't all, aside from useState Kent explained how to manage side effects with useEffect, how to create a custom hook so you are able to reuse logic in between components and also talked about refs and how it creates an instance of a variable without rerendering the component when it's value changes, just like we create instances using classes."